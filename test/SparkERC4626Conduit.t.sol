...
contract SparkERC4626ConduitDepositTests is SparkERC4626ConduitTestBase {
    ...
    function test_deposit_multiIlk_increasingShareValue() public {
        _assertTokenState({
            bufferBalance: 100 ether,
            vaultBalance: 0
        });
        ...
        deal(address(token), address(vault), 200 ether); // Shares value is doubled
        ...
        _assertTokenState({
            bufferBalance: 0,
            vaultBalance: 250 ether
        });

        uint256 sharesMinted = 50 ether * 100 ether / 200 ether;

        _assertVaultState({
            totalAssets:       250 ether,
            balance:           100 ether + sharesMinted,
            totalSupply:       100 ether + sharesMinted
        });

        assertEq(conduit.shares(address(token), ILK),  100 ether);
        assertEq(conduit.shares(address(token), ILK2), sharesMinted);
        assertEq(conduit.totalShares(address(token)),  100 ether + sharesMinted);
    }
...
contract SparkERC4626ConduitWithdrawTests is SparkERC4626ConduitTestBase {
    ...
    function test_withdraw_multiIlk_increasingShareValue() public {
        token.mint(buffer, 50 ether);
        conduit.deposit(ILK2, address(token), 50 ether);
        ...
        deal(address(token), address(vault), 100 ether); // Shares value is doubled

        assertEq(conduit.withdraw(ILK2, address(token), type(uint256).max), 100 ether);

        _assertTokenState({
            bufferBalance: 200 ether,
            vaultBalance: 0
        });
    ...
    function test_withdraw_multiIlk_decreasingShareValue() public {
        token.mint(buffer, 50 ether);
        conduit.deposit(ILK2, address(token), 50 ether);
        ...
        deal(address(token), address(vault), ERC20(address(token)).balanceOf(address(vault)) / 2);

        assertEq(conduit.withdraw(ILK2, address(token), type(uint256).max), 25 ether);

        _assertTokenState({
            bufferBalance: 125 ether,
            vaultBalance: 0 ether
        });
...
contract SparkERC4626ConduitGetTotalDepositsTests is SparkERC4626ConduitTestBase {
    ...
    function testFuzz_getTotalDeposits(
        ...
        external
    {
        interest = bound(interest, 0, type(uint96).max);
        ...
        deal(address(token), address(vault), depositAmount + interest);

        assertApproxEqAbs(conduit.getTotalDeposits(address(token)), depositAmount + interest, 10);
    }
...
contract SparkERC4626ConduitGetDepositsTests is SparkERC4626ConduitTestBase {
    ...
    function test_getDeposits() external {
        ...
        assertEq(conduit.getDeposits(address(token), ILK), 150 ether);
    }
        ...
        external
    {
        ...
        assertEq(conduit.getDeposits(address(token), ILK), depositAmount);

        deal(address(token), address(vault), depositAmount + interest);

        assertEq(conduit.getDeposits(address(token), ILK), depositAmount + interest);
    }
...
contract SparkERC4626ConduitAdminSetterTests is SparkERC4626ConduitTestBase {
    ...
    function test_setAssetEnabled() public {
        ...
        assertEq(token.allowance(address(conduit), address(vault)), type(uint256).max);
